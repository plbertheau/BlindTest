...
class JavaBlurProcess implements BlurProcess {
    ...
    private static final String TAG = JavaBlurProcess.class.getCanonicalName();
    ...
    @NonNull
    @WorkerThread
    public Bitmap blur(@NonNull Bitmap inputBitmap, @Nullable Bitmap outputBitmap, float radius, float multiplyFactor) throws BlurException {
//        Debug.log_d(DebugConfig.IMAGE_LIB, TAG, "starting java blur");
        int w = inputBitmap.getWidth();
        int h = inputBitmap.getHeight();
        int[] currentPixels = new int[w * h];
        inputBitmap.getPixels(currentPixels, 0, w, 0, 0, w, h);
        ...
        for (int i = 0; i < cores; i++) {
            horizontal.add(new BlurTask(currentPixels, w, h, (int) radius, cores, i, 1, 1f));
            vertical.add(new BlurTask(currentPixels, w, h, (int) radius, cores, i, 2, multiplyFactor));
        }

        try {
            StackBlurManager.EXECUTOR.invokeAll(horizontal);
        } catch (InterruptedException e) {
            throw new BlurException("Error executing the horizontal blur tasks", e);
        }

        try {
            StackBlurManager.EXECUTOR.invokeAll(vertical);
        } catch (InterruptedException e) {
            throw new BlurException("Error executing the vertical blur tasks", e);
        }

        if(outputBitmap == null){
            outputBitmap = Bitmap.createBitmap(currentPixels, w, h, Bitmap.Config.ARGB_8888);
        } else {
            outputBitmap.setPixels(currentPixels, 0, w, 0, 0, w, h);
        }
        return outputBitmap;
    }

    private static void blurIteration(int[] src, int w, int h, int radius, int cores, int core, int step, float multiplyFactor) {
        int x, y, xp, yp, i;
        int sp;
        int stack_start;
        int stack_i;
                ...
                for(y = 0; y < h; y++)
                {
                    src[dst_i] = (int)
                            ((src[dst_i] & 0xff000000) |
                                    ...
                                    ((((sum_g * mul_sum) >>> shr_sum) & 0xff) << 8) |
                                    ((((sum_b * mul_sum) >>> shr_sum) & 0xff)));
                    red = (int) (Color.red(src[dst_i]) * multiplyFactor);
                    green = (int) (Color.green(src[dst_i])* multiplyFactor);
                    blue = (int) (Color.blue(src[dst_i])* multiplyFactor);
                    src[dst_i] = Color.argb(Color.alpha(src[dst_i]), red, green, blue );
                    dst_i += w;
    ...
    private static class BlurTask implements Callable<Void> {
        ...
        private final float _multiplyFactor;

        public BlurTask(int[] src, int w, int h, int radius, int totalCores, int coreIndex, int round, float multiplyFactor) {
            _src = src;
            _w = w;
            _h = h;
            _radius = radius;
        ...
        @Override public Void call() throws Exception {
            blurIteration(_src, _w, _h, _radius, _totalCores, _coreIndex, _round, _multiplyFactor);
            return null;
        }
